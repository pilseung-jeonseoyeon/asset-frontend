...
// column (e.g. two fields in a flex row).
//
// 데스크톱도 같은 방식으로 바꿨다(2026-08-19, 사용자 지적 — "select box가 모달 안쪽으로 생겨서 모달에
// 스크롤이 걸린다. 밖으로 꺼내기로 했잖아"). 위 주석이 "out of scope, known gap"으로 남겨뒀던 바로 그
// 문제다: Modal.tsx의 기본 패널 스타일은 overflow:visible이지만 대부분의 호출부가 panelStyle로
// `maxHeight:86vh; overflow:auto`를 덮어쓰기 때문에(`grep -rn "panelStyle=" src/`), position:absolute인
// 이 패널은 모달 패널의 스크롤 박스에 갇혀 잘리고 모달 자체에 스크롤이 생겼다. DatePicker.tsx가 먼저
// 쓴 해법을 그대로 가져온다 — 트리거의 화면 좌표(rect)에 position:fixed로 앵커링하면 조상의 overflow
// 클리핑을 통째로 벗어나 팝오버가 모달 밖으로 넘쳐 떠 있을 수 있다. 폭은 DatePicker(고정 240px)와
// 달리 **트리거 자신의 폭**을 그대로 쓴다 — 기존 desktop 스타일이 left:0/right:0으로 필드 폭에 맞춰져
// 있었으므로 그 겉모습을 유지하는 값이다. 오른쪽 뷰포트 밖으로 나가지 않게 left는 클램프한다.
...
//
// o.meta/o.leading(2026-08-18 추가, QuickStockModal 계좌 드롭다운): 옵션에 보조 정보 한 줄(기관명)과
// 아이콘(BankIcon)을 함께 실을 수 있게 했다. key는 o.id가 있으면 그걸, 없으면 기존처럼 o.name으로
// 폴백한다(계좌처럼 이름이 겹칠 수 있는 목록에서 o.name만 쓰면 React key가 충돌한다).
//
// 렌더 분기는 옵션 하나 단위가 아니라 **드롭다운 전체(dd.options) 단위**다 — meta/leading을 쓰는
// 옵션이 하나도 없는 목록(가계부 카테고리·종목·기관·계좌 수정/추가·환전 등 기존 모든 호출부)은
// display:'block'인 예전 렌더를 그대로 쓴다(줄바꿈 허용, ellipsis 없음). 처음엔 o.name span에
// 항상 ellipsis(overflow:hidden + whiteSpace:nowrap)를 걸어 개별 옵션 단위로만 갈랐더니, "배달음식/
// 야식"처럼 자유 입력 옵션명이 있는 기존 목록에서 줄바꿈되던 이름이 전부 잘려나가는 회귀가 났다(리뷰
// 지적) — 옵션 하나라도 meta/leading을 쓰면 그 목록 전체를 새 2줄 레이아웃으로 통일해서, 아이콘이
// 있는 행과 없는 행(리뷰 지적 — leading 없는 행은 그 폭만큼 스페이서로 메워 시작선을 맞춘다)이 한
// 목록 안에 섞여도 정렬이 흐트러지지 않게 한다. ellipsis도 이 새 레이아웃에만 적용된다.
...
// 시작선을 맞춘다(리뷰 지적) — 지금 leading을 넘기는 유일한 호출부(QuickStockModal 계좌 드롭다운)가
  ...
  return (
    <>
      {/* minWidth:0 3곳(트리거 flex 컨테이너 + 아래 span) — 계좌 드롭다운처럼 dd.value가 "기관명 ·
          ...
          커진다(리뷰 지적). ellipsis로 자르려면 컨테이너부터 span까지 min-width 체인을 0으로 열어줘야
