// Source: secret/Asset Manager v14.dc.html L3695 `const formatNumber = (n) => n.toLocaleString('ko-KR')`.
 ...
 * 그대로 formatNumber()에 넣으면 주식 화면에 `6,665,311.125원`처럼 나왔다.
...
/**
 * 통화별 고정 소수 자릿수로 금액을 포맷한다. formatNumber()와 마찬가지로 통화 기호(₩/$)는 포함하지
 * 않는다 — 붙이는 건 호출부 몫이다. 원화는 소수점 없는 정수, 그 외(현재는 USD만 취급)는
 * 소수점 2자리로 고정한다. formatNumber()가 자릿수를 고정하지 않아 USD 금액이 `$77` / `$77.5` /
