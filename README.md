...
백엔드를 로컬에 띄우지 않아도 됩니다 — `pnpm dev`는 개발 서버 프록시를 통해 운영 API에 붙습니다
(그래서 로컬 화면에서 등록·삭제한 내용이 실제 데이터에 반영됩니다. 테스트용 계정으로 쓰세요).
로컬 백엔드를 쓰려면 `.env.local`에 `VITE_DEV_PROXY_TARGET=http://localhost:8080`을 적으면 됩니다.
