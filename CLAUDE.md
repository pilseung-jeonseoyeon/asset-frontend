...
- **자산 분류**: 서버 자산군은 현금 / 예적금 / 주식 / 가상자산 / 연금·기타 **5종**이며 각각 아이콘·금액·전체 대비 비중을 가집니다. 2026-08-27 라이브 OpenAPI 대조로 `DOMESTIC_STOCK`·`FOREIGN_STOCK`이 `STOCK` 하나로 합쳐진 것을 확인하고 프론트를 맞췄습니다(실제 증권계좌 하나가 원화·달러 예수금과 국내·해외 종목을 함께 담기 때문) — 국내/해외 주식을 계좌 유형으로 다시 가르지 마세요.
  **자산군별 추이**는 별도 엔드포인트가 아니라 총자산 추이 API에 계좌 유형 필터를 더한 것입니다 — `GET /dashboard/trend?type={AccountType}`(2026-08-27 백엔드 추가). 계좌 유형과 자산군이 1:1이라 성립합니다(`type=STOCK` → 주식 자산군 추이). **계좌 하나를 지정하는 파라미터는 없습니다** — 그 유형 계좌 전부의 합계이므로 계좌 상세(`AccountDetailModal`)에 "이 계좌의 추이"로 그리면 안 됩니다. 쓰는 자리는 자산군 상세(`AssetCategoryModal`) 하나뿐이고, 그래프는 `assetsView.ts`의 `buildAssetClassTrendPath`로 그립니다(대시보드의 `buildTrendChart`는 x축이 올해 1~12월 달력이라 6개월 창에 맞지 않습니다). 계좌별 추이는 백엔드 요청 항목으로 남아 있습니다.
