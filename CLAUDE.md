...
- [`docs/excel-import.md`](./docs/excel-import.md) — 가계부 엑셀 가져오기 서버 계약 요약(양식 시트·열 순서·엔드포인트·응답·에러 코드). 백엔드 소스와 대조해 적은 것이라, 실행 중 서버 OpenAPI에 올라오면 다시 대조
...
- **데이터는 서버에서 옵니다.** axios + React Query 기반 API 레이어가 `src/services/{domain}/`에 도메인별로 있고(`auth` `user` `institution` `account` `asset` `category` `transaction` `subscription` `stock` `trade` `exchange` `marketIndex` `goal` `dashboard` `notification` `export` `import` `connection`), 자산·가계부·주식 화면과 헤더 알림·자산 목표는 조회와 생성/수정/삭제가 모두 실제 API에 연결되어 있습니다. `import`(가계부 엑셀 가져오기)는 **한 행이라도 틀리면 전부 등록하지 않는(전체 롤백)** 서버 계약이라, 결과 화면도 "N건 등록" 아니면 "틀린 행 목록" 둘 중 하나입니다 — 계약은 `docs/excel-import.md`.
  ...
  **목업으로 남은 화면은 이제 없습니다**(2026-09-06 월간 리포트 오버레이까지 `GET /dashboard/reports`에 연결). 리포트는 `src/screens/Assets/modals/ReportOverlay.tsx`가 원본 시안대로 **9:16 스토리 카드 5장**(총자산 변화 → 자산 하이라이트 → 소비 → 전월·전년 비교 → 요약 카드)으로 그리고, 뷰모델은 `src/data/reportView.ts`입니다. 첫 장 제목 옆 화살표로 지난 정산월을 넘겨볼 수 있고, 마지막 장의 **'이미지로 저장'·'공유하기'는 지금 보이는 장을 1080×1920 PNG로 굽습니다**(`src/utils/reportExport.ts`, html-to-image를 누를 때 지연 로드). 이미지에 넣는 웹폰트는 문서의 @font-face 93개 전부가 아니라 **브라우저가 이미 내려받은 조각만**입니다 — 전부 넣으면 3.9MB SVG를 그리느라 수십 초 멈춥니다(2026-09-06 확인). 공유는 Web Share API(파일 공유)이고, 지원하지 않는 브라우저에서는 대신 다운로드합니다. **저장 규격은 스토리(1080×1920) 하나뿐입니다** — `ds_rules` §11-1이 피드(1080×1350)도 적어 두었지만 카드가 9:16 고정이라 피드 비율은 별도 레이아웃이 필요해 만들지 않았습니다(2026-09-06 결정). 필요해지면 사용자에게 먼저 확인하세요. **서버 비율 필드는 모수가 0이면 0이 아니라 `null`**이라 화면은 null을 '—'로 두고 0으로 바꾸지 않습니다. 정수 반올림으로 0%가 됐지만 금액은 움직인 경우 '1% 미만'으로 적습니다. 서버가 '유형 라벨'을 주지 않으므로 요약 카드 제목은 라벨 대신 실제 숫자로 만든 문장입니다. 문서형 한 장 레이아웃으로 바꿨다가 사용자 요청('기존 html 느낌으로')으로 스토리 형식으로 되돌렸습니다 — 다시 문서형으로 바꾸지 마세요. 대시보드 화면은 서버 통신(`src/services/dashboard`)과 뷰모델 변환(`src/data/dashboardView.ts`)을 거쳐 실제 API에 연결되어 있습니다(`mockDashboard.ts`는 삭제됨).
...
src/
  state/                 앱 전역 상태(AppState) — "모델" 레이어
    ...
    actions.ts              PATCH(부분 병합) / PATCH_FN(이전 상태를 받는 갱신 함수) 액션 타입
    reducer.ts               단순 병합 리듀서
    AppStateContext.tsx       Provider + useAppState() 훅
    selectors/                기능별 순수 헬퍼 함수(auth, datePicker, dropdown, entryDraft,
                              modal, nav, tabStyles, stockTabStyles) — 메모이즈하지 않은
                              일반 함수. 렌더마다 다시 계산해도 될 만큼 가볍다
  services/              API 통신 레이어. api.ts(axios + ApiError + unwrap), api.types.ts,
                         common.type.ts(도메인 공용 enum), queryKeys.ts(queryKeys 레지스트리),
  ...
  components/
    primitives/            원자 단위 UI 컴포넌트(Card, Icon, DonutChart, Treemap,
...
- **가계부 카테고리는 서버 리소스**입니다(`GET /categories`). 수입/저축/지출 3개 구분(`CategoryKind`) 아래 대분류가 있고, 그 아래 소분류가 붙습니다. **대분류는 서버 시드 고정이라 API로 만들 수 없고, 소분류만 추가·삭제할 수 있습니다.** 입력 폼은 배열 인덱스가 아니라 **`subcategoryId`(서버 id)** 로 선택을 추적합니다.
  ...
  **저축은 둘 다 필수**입니다 — 저축액이 들어간 계좌(`transferAccountId`)를 받지 않으면 출금만
  ...
  자산 구성만 바뀝니다).
- **자산 분류**: 서버 자산군은 현금 / 예적금 / 주식 / 가상자산 / 연금·기타 **5종**이며 각각 아이콘·금액·전체 대비 비중을 가집니다. 2026-08-27 라이브 OpenAPI 대조로 `DOMESTIC_STOCK`·`FOREIGN_STOCK`이 `STOCK` 하나로 합쳐진 것을 확인하고 프론트를 맞췄습니다(실제 증권계좌 하나가 원화·달러 예수금과 국내·해외 종목을 함께 담기 때문) — 국내/해외 주식을 계좌 유형으로 다시 가르지 마세요.
  **자산군별 추이 그래프는 화면에 없습니다**(2026-08-28 사용자 결정 — 자산군 상세의 "최근 6개월 추이" 칸을 걷어냈고, `buildAssetClassTrendPath`도 함께 삭제했습니다). API 자체는 남아 있습니다 — `GET /dashboard/trend?type={AccountType}`(2026-08-27 백엔드 추가)는 총자산 추이 API에 계좌 유형 필터를 더한 것이고, 계좌 유형과 자산군이 1:1이라 성립합니다(`type=STOCK` → 주식 자산군 추이). **계좌 하나를 지정하는 파라미터는 없으므로** 계좌 상세(`AccountDetailModal`)에 "이 계좌의 추이"로 그리면 안 됩니다. 추이 그래프를 다시 넣자는 이야기가 나오면 먼저 사용자에게 확인하세요. 현재 `useGetDashboardTrend`를 쓰는 화면은 대시보드 하나뿐입니다(`type` 옵션은 서버 계약을 그대로 반영해 훅에 남겨 둡니다). 계좌별 추이는 백엔드 요청 항목으로 남아 있습니다.
  **계좌 상세의 '최근 거래내역'은 두 리소스를 합친 목록입니다** — `GET /transactions`(가계부)와 `GET /trades`(매수·매도)를 프론트가 날짜순으로 병합합니다(`buildAccountActivity`). 매매는 가계부 거래를 만들지 않아 중복되지 않고, `GET /trades`는 주식·가상자산 계좌에서만 호출합니다.
  자산 화면에는 여기에 **'부동산' 칸이 하나 더** 붙어 총 6칸으로 보이지만, 이것은 서버 자산군이 아니라 `Assets.tsx`가 직접 그리는 "준비 중" 카드입니다(누르면 `RealEstateSoonModal`). **서버에 부동산 자산군이 생기기 전까지 `AssetClass` 유니언에 넣지 마세요** — 넣는 순간 도넛·트리맵·응답 파싱이 전부 있지도 않은 값을 다룰 수 있는 척하게 됩니다. 맵(트리맵) 뷰와 대시보드 도넛에는 부동산이 나오지 않습니다.
  **계좌 유형(`AccountType`)과 자산군(`AssetClass`)은 5종끼리 1:1 대응합니다** — 이름만 `ETC`(자산군) ↔ `PENSION_ETC`(계좌 유형)로 다릅니다. 예전의 세부 계좌 유형 10종(파킹통장·정기예금·증권계좌 등)과 국내/해외 주식 구분은 사라졌으니 되살리지 마세요. 매핑은 `src/data/assetsView.ts`의 `ASSET_CLASS_ACCOUNT_TYPE_PRESET`/`assetClassOfAccountType` 한 쌍이 정본입니다.
- **계좌 통화**: 계좌 표시 통화(`currency`)는 하나(`KRW` 또는 `USD`)지만 **금액 필드의 단위가 아니라 표기용**이고, **한 계좌가 원화 예수금과 달러 예수금을 동시에 가질 수 있습니다**(환전 전 원화가 남아 있는 증권 계좌). 등록 시 원화는 `initialBalanceKrw`(정수), 달러는 **`initialBalanceUsd`**(소수점 2자리)로 보내며, 둘 다 보내도 되고 한쪽만 보내도 되고 둘 다 생략해도 됩니다(생략하면 0). `initialBalanceUsd`를 보낼 수 있는 계좌는 **외화 표시 계좌 또는 주식·가상자산 계좌**이고(2026-08-27 OpenAPI 설명), 그 밖의 원화 계좌가 보내면 400 `INITIAL_BALANCE_CURRENCY_MISMATCH`입니다. 프론트 화면에는 통화 선택 UI가 없고 **항상 `KRW`를 보냅니다**(2026-08-27 주식 통합 전에는 해외주식 칩만 `USD`를 보냈습니다). 필드명이 `initialBalanceNative`가 아니라는 점에 주의하세요 — 그 이름은 서버 계약에 존재한 적이 없고, 그대로 보내면 달러 예수금이 조용히 누락됩니다(2026-08-20 라이브 OpenAPI 대조로 확인·수정). **환율은 프론트가 다루지 않습니다** — 서버가 두 원금을 입력값 그대로 보관하고 외화분의 원화 환산만 조회 시점 환율로 매번 계산하므로, 프론트에서 환율을 곱하면 이중 환산이 됩니다. '원금 대비 +N%' 배지의 기준값은 `initialBalanceKrw`가 아니라 **`totalPrincipalKrw`**(외화분까지 같은 시점 환율로 환산해 더한 값)입니다. 통화는 등록 후 수정할 수 없습니다(`PATCH`에 필드 자체가 없음). **잔액 정정을 막는 기준은 통화가 아니라 달러 예수금 유무(`initialBalanceUsd != null`)입니다** — 그런 계좌는 400 `BALANCE_ADJUSTMENT_NOT_SUPPORTED_FOR_FX`이고, 통화가 `USD`여도 아직 환전 전이라 달러 예수금이 없으면 정정할 수 있습니다(2026-08-30 OpenAPI 확인). 이 앱은 계좌를 **항상 `KRW`로 등록**하므로 달러 예수금이 있는 주식 계좌도 통화는 `KRW`입니다 — `currency === 'USD'`로 판별하면 정정 칸을 열어줬다가 저장 시점에 400을 맞습니다.

  **현재 예수금은 통화별로 내려옵니다**(2026-08-30 계약, 목록·상세 공통): `cashKrw`(현재 원화) / `cashUsd`(현재 달러) / `cashUsdKrw`(달러분 원화 환산액) / `usdKrwRate`(환산에 쓴 환율, 달러 없으면 `null`)이고 `balanceKrw = cashKrw + cashUsdKrw`입니다. **화면 표시에는 등록 시점 값(`initialBalance~`)이 아니라 `cash~`를 씁니다.** 원화 환산은 서버가 준 `cashUsdKrw`를 그대로 쓰세요 — `cashUsd × usdKrwRate`로 다시 계산하면 서버 반올림과 어긋납니다(`usdKrwRate`는 "어떤 환율로 환산했는지" 표기용). 참고로 `initialBalanceUsd`는 이름과 달리 OpenAPI 설명이 "현재 달러 예수금, `cashUsd`와 같은 값"으로 바뀌어 있어 **백엔드 확인이 필요한 항목**입니다 — 그래서 프론트는 이 필드를 화면에 그리지 않고 위 판별용으로만 씁니다.

  **`GET /accounts/{accountId}`만 응답이 한 겹 감싸져 있습니다**(`AccountDetailResponse`): `{ account, holdingValueKrw, totalValueKrw }`이고 `totalValueKrw = account.balanceKrw + holdingValueKrw`입니다. 목록·생성·수정·잔액정정 네 엔드포인트는 여전히 계좌를 그대로 돌려주므로, 상세를 부르는 곳에서만 `.account`를 꺼내야 합니다. 계좌 상세 화면의 대표 금액은 `balanceKrw`(예수금만)가 아니라 **`totalValueKrw`**입니다 — 주식 계좌에서 `balanceKrw`만 보여주면 보유 종목 평가액이 통째로 빠집니다.
