...
이 저장소에는 Monit 제품을 잘 아는 전담 개발자처럼 동작하도록 설정해둔 서브에이전트(특정 역할에 맞춰 미리 준비해둔 보조 작업자, `.claude/agents/`에 정의됨) 5명이 있습니다. 실제 개발팀에 화면 개발자와 리뷰어가 따로 있는 것처럼, 아래 상황에서는 직접 코드를 짜지 말고 반드시 해당 서브에이전트를 호출하세요.

1. **화면 UI를 새로 만들거나 수정할 때 → `frontend-developer`를 사용한다.** 컴포넌트 구현, 반응형 레이아웃, 인터랙션/상태 관리, API 연동이 필요한 모든 작업이 대상이며, 페이지 렌더링 속도(초기 청크 크기, 불필요한 리렌더, 체감 속도)까지 함께 챙깁니다.
2. **화면 구현이나 수정이 끝난 뒤 → 리뷰어 4명에게 반드시 검토를 받는다.** 사용자가 따로 요청하지 않아도 구현이 끝나면 자동으로 리뷰를 돌립니다. 네 리뷰는 관점이 다르므로 모두 실행합니다(서로 독립적이라 병렬 호출 가능).
   - `ui-ux-reviewer` — 최신 모바일 기기(아이폰·갤럭시)와 잘 만든 앱들의 UX 관례를 기준으로 사용자 편의성을 점검합니다. 규칙 검사에 그치지 않고, 제품 오너처럼 "어떻게 하면 더 편해지는가"에 대한 개선 제안까지 내놓습니다.
   - `strict-code-reviewer` — 코드를 읽으며 버그, 성능 저하 요인을 점검합니다.
   - `frontend-security-reviewer` — XSS, 인증정보(토큰·쿠키) 취급, 민감정보 노출, 브라우저 보안을 점검합니다.
   - `frontend-test-engineer` — 변경이 깨질 수 있는 테스트 시나리오를 설계하고 엣지 케이스(빈 값·경계값·정산월 경계·이상 입력 등)를 체계적으로 검증합니다. 테스트 러너가 생기면 테스트 코드 작성도 담당합니다.
   - 화면과 무관한 코드 변경이면 `ui-ux-reviewer`는 생략하고 나머지 리뷰어만 돌려도 됩니다.
3. **리뷰에서 문제가 발견되면 그대로 끝내지 않고 수정 루프를 돈다.** 리뷰어들이 지적한 내용을 바탕으로 `frontend-developer`가 다시 수정하고, 심각한 지적이 있었다면 수정 후 해당 리뷰어에게 재검토를 받습니다. "문제 없음"이 나올 때까지, 혹은 더 이상 고칠 게 없다고 판단될 때까지 이 과정을 반복합니다.
...
- **계좌 통화**: 계좌 하나는 통화 하나만 가집니다(`KRW` 또는 `USD`). 등록 시 통화에 맞는 원금 필드 **한쪽만** 보내야 하고(`KRW`→`initialBalanceKrw` 정수, `USD`→`initialBalanceNative` 소수점 2자리), 둘 다 보내거나 둘 다 안 보내면 400 `INITIAL_BALANCE_CURRENCY_MISMATCH`입니다. **환율은 프론트가 다루지 않습니다** — 서버가 등록 시점 환율로 `principalKrw`를 확정하고 현재 평가액(`balanceKrw`)은 조회할 때마다 환산하므로, 프론트에서 환율을 곱하면 이중 환산이 됩니다. 통화는 등록 후 수정할 수 없고(`PATCH`에 필드 자체가 없음), **외화 계좌는 잔액 정정도 지원하지 않습니다**(400 `BALANCE_ADJUSTMENT_NOT_SUPPORTED_FOR_FX`).
